	...
	for(int m=0;m<edge_list.size();m++)
	{
		...
		for(int l=0;l<edge_list[m].size();l++)
		{
			...
			if(n>m)
			{
				...
				{
					if(edge_super[supernode1][k]==supernode2)//edge_super already record this link, we only need to add weights
					{
						flag=1;
						weight_super[supernode1][k]+=weight[m][l];//no noise noise*weight[m][l]=weight[m][l]
						for(int k2=0;k2<edge_super[supernode2].size();k2++)
						{
							if(edge_super[supernode2][k2]==supernode1)
							{
								weight_super[supernode2][k2]+=weight[m][l];
								break;
							}
						}
						break;
				...
				if (flag==0 && supernode1!=supernode2 )//edge_super do not record this link, we need to initial this link
				{
					edge_super[supernode1].push_back(supernode2);
					edge_super[supernode2].push_back(supernode1);
					double wij=weight[m][l]; //no noise noise*weight[m][l]=weight[m][l]
